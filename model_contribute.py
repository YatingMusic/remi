...
def data_augmentation(midi_path, transposition_range=[0], tempo_stretching_range=[1.0]):
    midi_obj = miditoolkit.midi.parser.MidiFile(midi_path)
    notes = midi_obj.instruments[0].notes
    tempos = midi_obj.tempo_changes
    new_midi = []

    for pitch in transposition_range:
        for tempo in tempo_stretching_range:
            new_notes_list = []
            new_tempo_list = []
            for i in notes:
                note = i
                note.pitch += pitch
                new_notes_list.append(i)
            for i in tempos:
                temp = i
                temp.tempo *= tempo
                new_tempo_list.append(temp)
            new_midi_obj = midi_obj
            new_midi_obj.instruments[0].notes = notes
            new_midi_obj.tempo_changes = new_tempo_list
            new_midi.append(new_midi_obj)
    return new_midi
...
class PopMusicTransformer(object):
    ...
    # initialize
    ########################################
    def __init__(self, checkpoint, use_chord=True, is_training=False, train_from_scratch=True):
        # load dictionary
        ...
        if self.is_training:
            ...
            self.batch_size = 1
        self.checkpoint_path = '{}/model_latest'.format(checkpoint)
    ...
    def generate(self, n_target_bar, temperature, output_path, prompt=None):
        ...
        if prompt:
            events = extract_events(prompt, self.use_chord)
    ...
    def finetune(self, training_data, output_checkpoint_folder):
        ...
        for e in range(200):
            ...
            self.saver.save(self.sess, "{}/model_latest".format(output_checkpoint_folder))
            # stop
